...
If your Serverless Postgres provider rejects `pg_search` during deployment, set
`DATABASE_SEARCH_MODE=basic`. This skips the ParadeDB BM25 migrations and uses basic PostgreSQL
keyword search fallback. Search remains available, but ranking quality and performance on large
datasets are lower than the BM25 path.
