...
<Steps>
  ...
  <Tabs items={['Serverless Postgres', 'Node Postgres']}>
    <Tab>
      ...
      # Required for Neon free/serverless Postgres when pg_search is unavailable.
      # This skips ParadeDB BM25 migrations and uses basic PostgreSQL search fallback.
      DATABASE_SEARCH_MODE=basic
      ```

      <Callout type={'warning'}>
        Some Serverless Postgres providers, including current Neon environments, may reject the
        `pg_search` extension used by ParadeDB BM25 search. Set `DATABASE_SEARCH_MODE=basic` when
        using Neon if deployment fails with `extension "pg_search" is deprecated and no longer
        allowed`. Basic mode keeps keyword search available with lower ranking quality and weaker
